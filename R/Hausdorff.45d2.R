Hausdorff45 = function(a,b){
  ...
  H_vec = NULL
  j = 1
  begining = ifelse(ay[1] != 0, 1, min(2, m))
  while (T) {
    if(j > b_len) break
  ...
  H_vec2 = NULL
  i = 1
  begining = ifelse(by[1] != 0, 1, min(2, m))
  while (T) {
    if(i > a_len) break
    if(ax[i]>bx[begining]) break
    i = i + 1
  }
  
  for(j in begining:(b_len-1)){
    
    H_temp = by[j] - ay[max(i-1,1)] + ifelse(i%%2,1,-1)*(bx[j] - ax[max(i-1,1)])
    if((H_temp<0) == (j%%2)) H_vec2 = c(H_vec2, H_temp)
    
    if(j < (b_len-1)){
      while (T) {
        if(i > m) break
        if(ax[i]>bx[j+1]) break
        i = i + 1
      }
    }
  }
