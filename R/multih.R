Hausdorff2d <- function(x,y,tol=1e-6){
  if(is.list(x)){
  ...
  x_rectl <- list(sort(c(x1-tol,x1,M)),
                  sort(c(x2-tol,x2,M)))
  ...
  for (xi in 1:n1) {
    for (yi in 1:n2) {
      if( (z_x[2*yi,2*xi]!=z_x[2*yi,2*xi-1])&&(z_x[2*yi,2*xi]!=z_x[2*yi-1,2*xi]) ) {
        ...
        z_y <- z_yx[yi,xi]
        x_jump <- c(x_rectl[[1]][xi],x_rectl[[2]][yi])
    ...
    if( z_1_jump!=z_4_jump ){
      z_y <- z_yx[n2+1,xi]
      x_jump <- c(x_rectl[[1]][xi],M)
      x_boundary <- rbind(x_boundary,c(x_jump, z_1_jump, rep((z_1_jump+z_4_jump)/2,2), z_4_jump,z_y))
    }
  }
  for(yi in 1:n2) {
  ...
  for (xi in 1:n1) {
    for (yi in 1:n2) {
      if( (z_y[2*yi,2*xi]!=z_y[2*yi,2*xi-1])&&(z_y[2*yi,2*xi]!=z_y[2*yi-1,2*xi]) ) {
        ...
        y_jump <- c(y_rectl[[1]][xi],y_rectl[[2]][yi])
  ...
  for (i in 1:nrow(x_jumps)) {
    if(x_jumps[i,3]>x_jumps[i,7]) projection_x <- rbind(projection_x, c(x_jumps[i,1:2]+x_jumps[i,3],x_jumps[i,1:3],1))
    if((x_jumps[i,7]>x_jumps[i,6]+1e-14)&&(abs(x_jumps[i,6]-x_jumps[5])>1e-14)) projection_x <- rbind(projection_x, c(x_jumps[i,1:2]+x_jumps[i,6],x_jumps[i,1:2],x_jumps[i,6],2))
  }
  
  ##### compute H
  return(hsearch_Rcpp(projection_x,projection_y, boundary = F))
}
